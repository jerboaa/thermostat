...
/**
 */
public class JarLister extends PathProcessor {

    public static final String CORE = "Thermostat Core Libraries";
    ...
    protected void process(Path path) {
        ...
        if (parentDirectory.equals("libs")) {
            parentDirectory = CORE;
        }

        List<Path> pluginPaths = paths.get(parentDirectory);
        if (pluginPaths == null) {
            pluginPaths = new ArrayList<>();
            paths.put(parentDirectory, pluginPaths);
        }
