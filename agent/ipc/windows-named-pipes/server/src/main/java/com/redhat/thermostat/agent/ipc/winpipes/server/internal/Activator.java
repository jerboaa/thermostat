...
public class Activator implements BundleActivator {
    ...
    public Activator() {
        this(new IPCServiceHelper());
    }
    
    Activator(IPCServiceHelper helper) {
        this.helper = helper;
    }

    public void start(final BundleContext context) throws Exception {
        // Register the IPC properties provider
        context.registerService(IPCPropertiesProvider.class.getName(), new WinPipesIPCPropertiesProvider(), null);
        transport = helper.createTransport();

        // now that it's started, register the server service
    ...
    static class IPCServiceHelper {
        WinPipesServerTransport createTransport() {
            return new WinPipesServerTransport();
        }
    }
}
