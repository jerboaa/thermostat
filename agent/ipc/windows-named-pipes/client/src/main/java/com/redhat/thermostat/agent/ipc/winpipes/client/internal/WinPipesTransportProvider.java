...
public class WinPipesTransportProvider implements ClientTransportProvider {

    @Override
    public ClientTransport create(IPCProperties props) throws IOException {
        if (!(props instanceof WinPipesIPCProperties)) {
            throw new IOException("Unexpected IPC properties for 'winpipes' type");
        }
        return new WinPipesTransportImpl((WinPipesIPCProperties)props);
    }
