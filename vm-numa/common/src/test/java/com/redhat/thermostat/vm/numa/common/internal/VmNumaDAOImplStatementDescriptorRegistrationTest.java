...
public class VmNumaDAOImplStatementDescriptorRegistrationTest {
    ...
    public void serviceLoaderCanLoadRegistration() {
        ...
        expectedClassNames.add(VmNumaDAOImplStatementDescriptorRegistration.class.getName());
        ServiceLoader<StatementDescriptorRegistration> loader = ServiceLoader.load(StatementDescriptorRegistration.class, VmNumaDAOImplStatementDescriptorRegistration.class.getClassLoader());
        List<StatementDescriptorRegistration> registrations = new ArrayList<>(1);
        StatementDescriptorRegistration vmNumaReg = null;
        for (StatementDescriptorRegistration r: loader) {
            assertTrue(expectedClassNames.contains(r.getClass().getName()));
            if (r.getClass().getName().equals(VmNumaDAOImplStatementDescriptorRegistration.class.getName())) {
                vmNumaReg = r;
            }
            registrations.add(r);
        }
        ...
        assertNotNull(vmNumaReg);
        assertEquals(5, vmNumaReg.getStatementDescriptors().size());
    }
}
